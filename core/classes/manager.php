 ...
 */
define('MAESTRO_VERSION', 'Maestro 1.0');
...
//    public $application;
    ...
    public static function getThemePath($theme = '') {
        ...
        if ($path == '') {
            $path = self::getPublicPath(self::$instance->getApp(), '', 'themes/' . self::$instance->getTheme());
            if (!is_dir($path)) {
                $path = self::getPublicPath('', '', 'themes/' . self::$instance->getTheme());
            }
            ...
            if ($last == '*') {
                ...
                if (is_array($files)) {
                    foreach ($files as $file) {
                        ...
                        $class = str_replace('.' . $pathinfo['extension'], '', $pathinfo['filename']);
                        $x = str_replace('*', '', $namespace) . $class;
                        if (!self::$instance->classAlias[$x]) {
                            class_alias($x, $class);
                            self::$instance->classAlias[$x] = $class;
                        }
                    }
                }
            } else {
...
//        return self::$instance->getBaseURL($absolute) . (self::$instance->java ? '' : '/apps') . '/' . $app . '/public' . ($file ? '/' . $file : '');
    ...
    public static function getThemeURL($file = '') {
        ...
        if (is_dir($path)) {
            $url = self::$instance->getAbsoluteURL("apps/{$app}/public/themes/{$theme}/{$file}");
        } else {
    ...
    public static function isHostAllowed() {
        ...
        foreach ($this->getOptions('hosts.allow') as $h) {
            if ($REMOTE_ADDR == $h) {
                $returnValue = true;
            }
     ...
     *
     */
    public static function traceDump($msg, $file = false, $line = 0) {
        return self::$instance->getObject('trace')->traceDump($msg, $file, $line);
    }
